...
function schedule(string) {
  ...
  string.replace(/ /g,'').split(';').map(s => s.split(':')).map(S => {
    ...
    days.split('-').forEach(day => {
      result[day] = blocks.split(',').map(Number);
    });
  });
  return result;
}

function validTeacher(name) {
  return name.toLowerCase() !== '(sin profesores)';
}

module.exports = function(qs, options) {
  options = options || {};
  ...
  return request(query(qs)).then($ => {
    ...
    function handler(tr) {
      ...
      course.teachers = (teachers) ? teachers.split(',').filter(validTeacher).map(name => {
        name = name.trim();
        return {
          name: name,
      ...
      $('table', values[13]).children().each((_, subtr) => {
        ...
        const shc = hashSchedule[type] = hashSchedule[type] || {
          modules: {},
          location: {
      ...
      course.schedule = Object.keys(hashSchedule).map(type => {
        const object = hashSchedule[type];
        return {
          identifier: type,
          location: object.location,
          modules: Object.keys(object.modules).map(day => {
            return {
              day: day,
              hours: object.modules[day],
            }
          }),
        }
      });
      ...
      // Parallel requests
      return Promise.all([information, requisites])
      .then(results => results.map(r => (r && r.isFulfilled()) ? r.value() : null))
      .then(results => {
    ...
    return Promise.map(rows, handler, { concurrency: options.concurrency });
  });
}
